...
## Следующий логичный шаг: эпик 06 — наблюдатель

1. Прочитать **`docs/business/epic-06-spectator.md`** и чеклист; часть сценариев уже есть в коде (`spectator:join`, `?watch=1`) — сверить пробелы с бизнес-требованиями.
2. При необходимости обновить **`docs/dev/API.md`** и клиент/сервер под оставшиеся пункты эпика.

Зависимости: эпики **07–09** — по PRD; эпик **05** (бот) закрыт в `main`.
...
- [ ] Открыть **`docs/business/epic-06-spectator.md`** и чеклист эпика.
- [ ] Сверить **`docs/dev/API.md`** и код: `spectator:join`, лобби, клиент зрителя.
...
> Продолжи Tennis 1v1 с **эпика 06** (наблюдатель). Прочитай `docs/dev/AGENT_HANDOFF.md`, затем `docs/business/epic-06-spectator.md` и чеклист. После работы — пройти чеклист эпика 06. В `main` уже эпики 01–05.
