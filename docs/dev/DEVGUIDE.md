...
Гостевая сессия (`POST /auth/guest`) по умолчанию пишет refresh-токен в **PostgreSQL**. Если БД недоступна, а **`NODE_ENV` не `production`**, сервер использует **in-memory** хранилище refresh-токенов (предупреждение в логе) — можно войти как гость и сыграть с ботом без Postgres. Для отключения: **`DISABLE_DEV_MEMORY_REFRESH=1`**. В production без БД гость по-прежнему не создаётся.
