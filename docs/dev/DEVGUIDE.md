...
Гостевая сессия (`POST /auth/guest`) по умолчанию пишет refresh-токен в **PostgreSQL**. Если БД недоступна, а **`NODE_ENV` не `production`**, сервер использует **in-memory** refresh и при любой ошибке основного пути — полный fallback создания гостя в памяти (предупреждение в логе). В **`NODE_ENV=production`** (часто Docker на сервере) без БД добавьте в `server/.env`: **`ALLOW_OFFLINE_GUEST=1`**. Отключить всё: **`DISABLE_DEV_MEMORY_REFRESH=1`**.
